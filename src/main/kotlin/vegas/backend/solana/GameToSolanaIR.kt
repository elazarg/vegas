...
private fun buildStateStruct(
    ...
    // Core state
    fields.add(SolanaField("creator", Pubkey))
...
private fun buildErrors(): List<SolanaError> {
    return listOf(
        ...
        SolanaError("CannotClose", "Cannot close game yet"),
...
private fun buildInstructions(
    ...
    list.add(buildCloseInstruction(roles))
...
private fun buildInitInstruction(stateStruct: SolanaAccountStruct): SolanaInstruction {
    ...
    return SolanaInstruction(
        ...
        body = listOf(
            Assign(FieldAccess(Var("game"), "creator"), MethodCall(Var("signer"), "key", emptyList())),
...
private fun buildCloseInstruction(roles: List<RoleId>): SolanaInstruction {
    val body = mutableListOf<SolanaStmt>()
    body.add(Let("now", I64, ClockTimestamp))

    // Condition 1: Abandoned (timeout + nobody joined)
    val anyJoined = roles.indices.map { Index(FieldAccess(Var("game"), "joined"), IntLit(it.toLong())) as SolanaExpr }
        .reduce { a, b -> Binary(BinaryOp.OR, a, b) }
    val timedOut = Binary(BinaryOp.GT, Var("now"), Binary(BinaryOp.ADD, FieldAccess(Var("game"), "last_ts"), FieldAccess(Var("game"), "timeout")))
    val abandoned = Binary(BinaryOp.AND, timedOut, Unary(UnaryOp.NOT, anyJoined))

    // Condition 2: Finished (finalized + all claimed)
    val allClaimed = roles.indices.map { Index(FieldAccess(Var("game"), "claimed"), IntLit(it.toLong())) as SolanaExpr }
        .reduce { a, b -> Binary(BinaryOp.AND, a, b) }
    val finished = Binary(BinaryOp.AND, FieldAccess(Var("game"), "is_finalized"), allClaimed)

    body.add(Require(Binary(BinaryOp.OR, abandoned, finished), SolanaError("CannotClose", "Cannot close game yet")))
...
private fun buildTimeoutInstruction(role: RoleId, roleMap: Map<RoleId, Int>): SolanaInstruction {
    ...
    // Set bailed = true. Do NOT update last_ts to allow multiple timeouts from same inactivity period.
    ...
    return SolanaInstruction(
        ...
        accounts = listOf(
            ...
            // Removed signer to allow permissionless timeout
        ),
        params = emptyList(),
        body = body
    )
...
private fun buildFinalizeInstruction(
    ...
    // Auto-claim if amount is 0
    roles.forEachIndexed { i, role ->
        body.add(SolanaStmt.If(
            Binary(BinaryOp.EQ, Index(FieldAccess(Var("game"), "claim_amount"), IntLit(i.toLong())), IntLit(0)),
            listOf(Assign(Index(FieldAccess(Var("game"), "claimed"), IntLit(i.toLong())), BoolLit(true)))
        ))
    }
...
private fun buildClaimInstruction(role: RoleId, roleIdx: Int): SolanaInstruction {
    ...
    return SolanaInstruction(
        ...
        accounts = listOf(
            ...
            SolanaAccountMeta("recipient", "SystemAccount<'info>", isMut = true, constraints = listOf(
                "#[account(address = game.roles[$roleIdx] @ ErrorCode::Unauthorized)]"
            )),
            SolanaAccountMeta("system_program", "Program<'info, System>")
        ),
        params = emptyList(),
...
private fun translateType(t: Type): SolanaType = when (t) {
    ...
    is Type.SetType -> {
        if (t.values.all { it in 0..255 }) U8 else I64
    }
}
