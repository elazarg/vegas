...
| OddsEvensShort: payoffs sum to 200 | Conservation property | Concurrent joins |
...
### 9.2 Bug Discovered and Fixed: Concurrent Join Actions

During test development, a bug was discovered and fixed in `LocalSession.legalMoves()`.

**Symptom**: PayoffEvaluationTest initially failed for OddsEvensShort because the game never reached terminal state. The session looped indefinitely on the join phase.

**Investigation**: Debug comparison between Prisoners and OddsEvensShort revealed:

- **Prisoners**: Join actions are sequential (`join A() $ 100; join B() $ 100;`), so each is in its own frontier
- **OddsEvensShort**: Join actions are concurrent (`join Odd() $ 100 Even() $ 100;`), so both are in the same frontier

When both joins are in the same frontier:
1. `legalMoves()` returns both join moves
2. User submits one join (e.g., Odd's)
3. `LocalSession.submitMove()` tracks it in `submittedZeroParam` but waits for Even's join before finalizing
4. `legalMoves()` is called again, but it still returns both joins (including the already-submitted one)
5. User submits Odd's join again → infinite loop

**Root cause**: `LocalSession.legalMoves()` did not filter out zero-parameter actions that had already been submitted in the current frontier.

**Fix applied** (LocalRuntime.kt:30-32):
```kotlin
...
**Impact of fix**:
- OddsEvensShort now correctly reaches terminal state
- All games with concurrent zero-parameter actions (joins) now work correctly
- All 466 tests pass
