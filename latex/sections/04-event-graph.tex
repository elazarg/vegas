...
The owner of a node always observes their own writes; for owners
this includes \emph{committed} fields as well as the
public/revealed ones, since a player knows the value they
...
In practice the typechecker's pre-IR desugaring strips fields
written by the current node from the guard's scope, so the
``owner sees their own committed fields'' branch of the
visibility rule matters mostly for guards that reference an
actor's \emph{earlier} private writes from elsewhere in the
protocol.
