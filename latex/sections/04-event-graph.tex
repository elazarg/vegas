...
This section introduces the EventGraph: the finite, visibility-aware
directed acyclic graph that all Vegas backends read.  The graph is
the structural core of the language --- everything from front-running
mitigation to information-set construction to bail-out subgames is
expressed as a property of, or a rewrite on, this graph.
...
produces the EventGraph below.  The two \TT{join} statements share
one source phase (the source has \TT{join Host()\ldots Guest()}
combined in a single top-level statement) and are therefore
concurrent; the remaining statements are separate phases and pick
up phase-order edges between them, so the rest of the graph is a
chain.  Squares are \COMMIT writes, hexagons are \REVEAL writes,
ovals are \PUBLIC writes, and the diamond is the terminal
\TT{withdraw} sink.  The commit-reveal expansion pass
(\S\ref{sec:commit-reveal}) does nothing here, because none of the
public yields are concurrent with each other.

\begin{center}
\begin{tikzpicture}[
  every node/.style={font=\small},
  ev/.style={draw, rounded corners, inner sep=2pt, minimum width=2.3cm,
             minimum height=4mm, fill=blue!6},
  cnode/.style={draw, inner sep=2pt, minimum width=2.3cm, minimum height=4mm,
             fill=red!10},
  rv/.style={draw, regular polygon, regular polygon sides=6,
             inner sep=0pt, minimum width=2.4cm, minimum height=5mm,
             fill=green!10},
  sink/.style={draw, diamond, inner sep=2pt, minimum width=2.0cm, fill=gray!8},
  arr/.style={-{Stealth[length=2mm]}, thick},
]
\node[ev]  (jH) at (-3,2.5) {join Host};
\node[ev]  (jG) at ( 3,2.5) {join Guest};
\node[cnode]  (cm) at (-3,1)   {commit Host.car};
\node[ev]  (yd) at ( 3,1)   {yield Guest.d};
\node[ev]  (yg) at ( 0,-0.5) {yield Host.goat};
\node[ev]  (ys) at ( 0,-2)   {yield Guest.switch};
\node[rv]  (rv) at ( 0,-3.5) {reveal Host.car};
\node[sink] (wd) at ( 0,-5)   {withdraw};
...
\begin{itemize}\itemsep1pt
\item \TT{join Host} and \TT{join Guest} are the only concurrent
  pair: they share a phase, neither reaches the other, and the
  Solidity contract accepts the corresponding two transactions in
  either order.
\item \TT{reveal Host.car} is enabled only after Guest has
  switched.  In any configuration where the reveal is on the
  frontier, every prior decision has been made.  Guest cannot
  switch \emph{after} seeing the car location, and Host cannot
  revise the car location.
\item \TT{yield Host.goat} reads \TT{Guest.d} (the guard
  \TT{Host.goat != Guest.d}), which would force the order
  \TT{yield Guest.d} $\to$ \TT{yield Host.goat} even if the two
  statements were in the same phase.  Here, phase order
  contributes the same edge.
\end{itemize}

Sequential graphs like this are the typical shape of textbook
turn-taking games; concurrency-driven rewriting becomes interesting
when multiple roles \emph{share} a yield statement, as in the
Vickrey example treated in the next section.
...
computes it; the formula is the only place where ``observability''
is defined.
