...
mixed-strategy equilibrium for Monty Hall.  Nothing in the
...
\item \emph{Automatic commit-reveal insertion}
  ...
  themselves.  Within the protocol, protection against
  observation-before-action MEV is then a structural property of
...
\item \emph{A family of backends} (\S\S\ref{sec:solidity}--\ref{sec:other-backends})
  ...
  protocol description; a Bitcoin/Lightning backend for the
  two-player fragment; a Coq/Lean ``Diamond DAG'' witness-record
  encoding that captures the per-step constraint bundle as a
  dependent type.
