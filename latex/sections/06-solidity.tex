...
\begin{itemize}\itemsep1pt
\item \emph{Bail is precise.}  Only a role whose missing action
  is named by some \TT{depends} clause can be bailed, and only
  when the global window has expired.  A role that has completed
  everything required of it cannot be bailed by a later caller;
  a fair-playing caller cannot be bailed by their own attempt to
  act.
\item \emph{Bail is one-way.}  Once \TT{bailed[\textit{R}] =
  true}, the \TT{by(R)} modifier rejects any subsequent action
  from \TT{R}.  This matches the language-level treatment of
