...
\paragraph{Chance nodes.}  Actions owned by a role declared
with \TT{random} (\S\ref{sec:language}) are emitted as Gambit
chance moves with a uniform distribution over the action's
domain --- the current source language has no syntax for
non-uniform distributions.  Each chance event is a node of its
own, with the same simultaneity treatment as a player move.
