...
Two simple simplifications are run on the produced tree before
emission.
...
yield $12^3 = 1728$ branches at the reveal frontier.  This is
fine for the example library; it is not what one would deploy
against a competitive-bidding mechanism with millions of bid
levels.  Vegas's contract-side bid types are typically small
because Gambit needs them small; the deployed contract can carry
larger ranges, but then Gambit-side analysis becomes infeasible.
This is the explicit tradeoff implied by ``the same source for
both views''.
