...
A backend (\file{backend/bitcoin/}) targets the two-party
Lightning channel for the subset of Vegas games with exactly
two strategic roles.  Each EventGraph node maps to a state in a
per-game state machine; the backend emits the full protocol
state machine, the role active at each state, the legal
transitions, and the per-state abort balances --- ready to drive
a Lightning runtime.  The two pieces still out-of-band are the
on-chain Bitcoin script emission and the Lightning-node
adapter; a two-party Vegas game compiles to a deployable
state-machine artifact today, modulo that adapter.

This backend also serves as an existence proof that the
EventGraph abstraction can lift to a non-EVM execution layer
with completely different cryptographic primitives.
...
Chance nodes in the emitted MAID consume the same per-node
distribution metadata as the Gambit backend.  A chance node with
an explicit \TT{$\sim$ uniform/weighted} annotation lands in the
MAID's CPD verbatim; a chance node without an explicit dist falls
back to a uniform CPD over the parameter's domain.  Self-only
guards are projected statically into a renormalised CPD: a
\TT{where} that excludes some support values yields a CPD whose
mass is concentrated on the surviving values.  An unsatisfiable
self-only guard (no surviving values) is reported as a static
error.

The MAID backend has a real expressiveness limit on
\emph{contextual} guards.  In plain MAIDs, a decision node's
action set is a single static domain; there is no place to say
``the legal action set depends on the parents' values''.  A
Vegas guard like \TT{where Host.goat != Guest.d} has nowhere to
go in such a MAID: the encoding would silently drop the
constraint.  The MAID emitter therefore refuses to emit for any
game whose guards reference fields written by other nodes.
Supporting context-dependent domains in some MAID extension is
listed in \file{TODO.txt}.
...
\item \TT{INDEPENDENT} (\TT{lean-independent} /
  ...
  ancestor witnesses wrapped in a \TT{Maybe} carrier (so a
  trace can model a partial play in which some prior actor's
  contribution is absent) and an \TT{isPresent} discipline on
  reveals.
