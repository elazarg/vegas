...
a state in a per-game state machine; the backend emits a
textual listing of the Lightning protocol moves and transitions
needed to realise the game (states, the role active at each
state, and the legal transitions between states).  No Bitcoin
script is generated --- the encoding to actual on-chain
opcodes, the integration with a Lightning node, and the
deployment work generally are all out-of-band.

The value of this backend in the present work is as an
existence sketch: the EventGraph abstraction can in principle
lift to a non-EVM execution layer with completely different
cryptographic primitives.  A proper Bitcoin/Lightning backend
