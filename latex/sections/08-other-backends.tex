...
shape of the encoding is worth describing in its own terms: it
treats every move and every guard as a type-level proof
obligation, and player strategies become functions on
typed prior witnesses.
...
The encoding is interesting for two reasons.  It is
self-contained: the Lean object can be passed to a theorem prover
without depending on a large semantics library; the encoding
\emph{is} the semantics.  And because each step is a function on
prior witnesses, the player's choice schedule appears as a
strategy in the strict type-theoretic sense --- an inhabitant of
a function space whose codomain is pinned by domain and
where-clause proofs.  This view is closer to the
