...
\TT{join} phase appears once, at the top of the game.  A role
listed in the join phase is committed to the game and has its
...
\paragraph{\TT{random}.}  A public chance event: a value is
sampled by a designated chance role from an explicit finite-support
distribution.
\begin{lstlisting}[style=vg]
random Chance(coin: bool);
\end{lstlisting}
The chance role is not a strategic player; \TT{random} writes
behave like \PUBLIC yields for visibility, and the Gambit backend
emits a chance node with the declared distribution.
...
withdraw (A.c && B.c) ? { A -> 100; B -> 100 }
       ...
       :                { A -> 90;  B -> 110 }
\end{lstlisting}
The compiler statically checks conservation against the constant
total deposit; payoff expressions may reference any public field.
...
The compiler does not assume the user has solved front-running:
it reads three simultaneous public moves and rewrites the
EventGraph so that each bidder commits to a hash first and reveals
later, with the reveal of any bidder gated on the commits of the
other two.  The resulting Solidity contract implements a sealed-bid
...
\mathit{ext} &::= \kw{join}\ \mathit{query}^+\ \TT{;}\ \mathit{ext} \mid (\kyield \mid \kreveal \mid \kcommit \mid \krandom)\ \texttt{or}\ \mathit{handler}\ \mathit{query}^+\ \TT{;}\ \mathit{ext} \\
