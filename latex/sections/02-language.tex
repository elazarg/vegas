...
There are five action kinds.  Each writes one or more fields
owned by its actor (for \TT{random}, the actor is a chance role).
...
A field declared with the \TT{hidden} prefix
(\TT{yield Host(car: hidden door)}) is private to its owner until
revealed.  An \TT{opt T} type marks a value that may be missing
(typically because the actor took the \TT{Quit} branch); the
language requires \TT{withdraw} expressions to handle the
\TT{null} case explicitly.
