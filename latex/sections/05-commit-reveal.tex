...
``two players \TT{yield} simultaneously'' gets, deployed to chain,
the commit-reveal protocol that the textbook would have insisted
on.
...
The rewrite lives at \file{ir/EventGraph.kt} as a static
method \TT{expandCommitReveal} on the \TT{EventGraph} class.  It
runs once, immediately after the initial IR construction, before
any backend gets to read the graph.  The implementation is about
sixty lines of Kotlin and is reachability-driven: it iterates over
the existing nodes, builds the risk-partner relation by checking
$\neg(\mathrm{reach}(a,b) \vee \mathrm{reach}(b,a))$ for each pair
of pure-public nodes, and then performs the local rewrite for each
risk-partner-bearing node, allocating fresh node ids above the
current maximum.  The resulting graph is re-validated by the
EventGraph constructor (it must still be acyclic, the new
\COMMIT-\REVEAL pairs must be in order, and the read-visibility
discipline must be preserved); the rewrite is constructed so that
all three properties are preserved by inspection, and the
post-rewrite validation is mostly defensive.
