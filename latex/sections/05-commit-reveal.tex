...
independent public moves from transaction-ordering adversaries.
The pass is small but the property it secures is one of the most
visible payoffs of using Vegas: a programmer who writes
``two players \TT{yield} without either observing the other first''
gets the commit-reveal protocol the textbook would have insisted
on, deployed on chain.

\subsection{The problem}
\label{sec:cr:problem}

Consider the one-phase specification
\begin{lstlisting}[style=vg]
yield A(x: bool) B(y: bool);
\end{lstlisting}
At the EventGraph level these are two PUBLIC writes on concurrent
nodes.  Read as a game, the analysis is unambiguous: A and B make
simultaneous strategic choices without observing each other.  Read
as a Solidity contract, the implementation has to be cautious.  If A's
...
specification's information-independence property is broken by the
deployment target's notion of transaction ordering.

This is a specific subclass of MEV --- \emph{observation-before-action}
on independent public moves --- and the canonical mitigation is well
...
group of originally independent public moves.
...
\begin{tikzpicture}[
  ...
  pu/.style={draw, rounded corners, inner sep=2pt, minimum width=1.4cm, minimum height=4mm, fill=blue!8},
  cnode/.style={draw, inner sep=2pt, minimum width=1.4cm, minimum height=4mm, fill=red!10},
  rv/.style={draw, regular polygon, regular polygon sides=6, inner sep=0pt,
             minimum width=1.5cm, minimum height=5mm, fill=green!10},
  bar/.style={draw, fill=gray!20, minimum width=4.4cm, minimum height=4mm,
              inner sep=2pt},
  arr/.style={-{Stealth[length=2mm]}, thick},
  arrg/.style={-{Stealth[length=2mm]}, thick, gray},
]
\node[pu] (b1) at (-0.9, 0)   {B1.b};
\node[pu] (b2) at ( 0.5, 0)   {B2.b};
\node[pu] (b3) at ( 1.9, 0)   {B3.b};
\node     at ( 0.5,-0.9) {\textit{before rewrite}};

\node[cnode] (c1) at ( 5.6, 0.8) {B1.b};
\node[cnode] (c2) at ( 7.0, 0.8) {B2.b};
\node[cnode] (c3) at ( 8.4, 0.8) {B3.b};
\node[bar]   (bar) at (7.0,-0.3) {commit phase complete};
\node[rv]    (r1) at ( 5.6,-1.5) {B1.b};
\node[rv]    (r2) at ( 7.0,-1.5) {B2.b};
\node[rv]    (r3) at ( 8.4,-1.5) {B3.b};
\node     at ( 7.0,-2.4) {\textit{after rewrite}};
\draw[arrg] (c1) -- (bar);
\draw[arrg] (c2) -- (bar);
\draw[arrg] (c3) -- (bar);
\draw[arr]  (bar) -- (r1);
...
The grey horizontal bar above stands for the join of the three
commits in the predecessor set of each reveal: every reveal lists
all three commits as predecessors, so we draw the dependency once
as a fan-in/fan-out around the conceptual ``commit phase
complete'' marker instead of as nine crossing arrows.
...
the rewritten graph as two independence-preserving frontier
stages (commits, then reveals); information sets are computed
from the new graph in the usual way, and they correctly reflect
the informational barrier.  No backend-specific commit-reveal
logic exists: the property is paid for once, in the IR, and
consumed everywhere.
