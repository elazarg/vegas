...
when it falls behind \TT{block.timestamp} by more than
\TT{TIMEOUT} seconds, the next modifier marks its named role as
bailed and resets the window.  Bail is one-way and attribution is
structural rather than analytical --- the role marked is the role
named by the next modifier, not necessarily the unique role that
failed to act.  A per-action deadline scheme would tighten the
attribution at the cost of substantially more state; the current
design is the simpler starting point.
