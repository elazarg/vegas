...
\paragraph{Fair-play liveness on unbounded state.}  The
conservation-enumeration pass (\S\ref{sec:liveness}) already
catches the bounded case --- a contextual guard that becomes
unsatisfiable in some reachable configuration is reported
statically.  What it does not cover is games whose state space
the enumerator cannot exhaustively explore (unbounded \TT{int}
parameters, very deep trees); an SMT-call fallback that types
the affected field as \TT{opt T} when satisfiability cannot be
established is the open piece, under the name \emph{fair-play
liveness}.
...
\paragraph{MAID contextual guards.}  The MAID emitter handles
self-only guards (\S\ref{sec:other-backends}) by projecting them
statically into a renormalised CPD.  Contextual guards --- whose
legal action set depends on prior nodes' writes --- cannot be
encoded in a plain MAID's static-domain model, so the emitter
refuses them.  A constrained-MAID extension is the long-term
route to lifting that restriction.
