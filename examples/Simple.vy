...
def withdraw_A():
    ...
    self.claimed_A = True
    payout: int256 = (6 + ((0 if self.done_A_c else 6 + 0 if True else 6) / (1 if self.done_A_c else 0 + 1 if True else 0) if ((1 if self.done_A_c else 0 + 1 if True else 0) > 0) else 1)) if self.done_A_c else 0 if (not self.done_A_c) else (6 + ((0 if self.done_A_c else 6 + 0 if self.done_B_c else 6) / (1 if self.done_A_c else 0 + 1 if self.done_B_c else 0) if ((1 if self.done_A_c else 0 + 1 if self.done_B_c else 0) > 0) else 1)) if self.done_A_c else 0 if (not self.done_B_c) else 9 if (self.A_c != self.B_c) else 3
    if payout > 0:
...
def withdraw_B():
    ...
    self._check_timestamp(Role.B)
    assert not self.bailed[Role.B], "you bailed"
    assert not self.actionDone[Role.B][4], "already done"
    self._check_timestamp(Role.A)
    if not self.bailed[Role.A]:
        assert self.actionDone[Role.A][4], "dependency not satisfied"
    assert (not self.claimed_B), "already claimed"
    self.claimed_B = True
    payout: int256 = (6 + ((0 if self.done_A_c else 6 + 0 if True else 6) / (1 if self.done_A_c else 0 + 1 if True else 0) if ((1 if self.done_A_c else 0 + 1 if True else 0) > 0) else 1)) if True else 0 if (not self.done_A_c) else (6 + ((0 if self.done_A_c else 6 + 0 if self.done_B_c else 6) / (1 if self.done_A_c else 0 + 1 if self.done_B_c else 0) if ((1 if self.done_A_c else 0 + 1 if self.done_B_c else 0) > 0) else 1)) if self.done_B_c else 0 if (not self.done_B_c) else 9 if (self.A_c == self.B_c) else 3
    if payout > 0:
        success: bool = raw_call(self.address_B, b"", value=convert(payout, uint256), revert_on_failure=False)
        assert success, "ETH send failed"
    self.actionDone[Role.B][4] = True
    self.actionTimestamp[Role.B][4] = block.timestamp
    self.lastTs = block.timestamp
