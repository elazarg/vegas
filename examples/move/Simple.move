module simple::simple {
    ...
    public entry fun move_B_3<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, c: bool) {
        assert!((sui::tx_context::sender(ctx) == instance.role_B), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_B = true;
        };
    ...
    public entry fun move_A_4<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, c: bool, salt: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_A), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_A = true;
        };
