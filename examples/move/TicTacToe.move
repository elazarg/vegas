module tictactoe::tictactoe {
    ...
    public entry fun join_O<Asset>(instance: &mut Instance<Asset>, payment: sui::coin::Coin<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!(!instance.joined_O, 100);
        instance.role_O = sui::tx_context::sender(ctx);
        instance.joined_O = true;
        sui::balance::join<Asset>(&mut instance.pot, sui::coin::into_balance<Asset>(payment));
        instance.last_ts_ms = sui::clock::timestamp_ms(clock);
    }

    public entry fun move_X_0<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!((sui::tx_context::sender(ctx) == instance.role_X), 101);
    ...
    public entry fun move_O_1<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!((sui::tx_context::sender(ctx) == instance.role_O), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
    ...
    public entry fun move_X_2<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, c1: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_X), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_X = true;
        };
    ...
    public entry fun move_O_3<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, c1: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_O), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_O = true;
        };
    ...
    public entry fun move_X_4<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, c2: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_X), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_X = true;
        };
    ...
    public entry fun move_O_5<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, c2: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_O), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_O = true;
        };
    ...
    public entry fun move_X_6<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, c3: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_X), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_X = true;
        };
    ...
    public entry fun move_O_7<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, c3: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_O), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_O = true;
        };
    ...
    public entry fun move_X_8<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, c4: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_X), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_X = true;
        };
    ...
    public entry fun move_O_9<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, c4: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_O), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_O = true;
        };
    ...
    public entry fun claim_X<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
    ...
    public entry fun claim_O<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!(instance.finalized, 110);
