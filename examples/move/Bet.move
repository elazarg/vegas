module bet::bet {
    ...
    public entry fun create_game<Asset>(timeout_ms: u64, ctx: &mut sui::tx_context::TxContext) {
        let instance = Instance<Asset> { id: sui::object::new(ctx), role_Gambler: 0x0, joined_Gambler: false, timeout_ms: timeout_ms, last_ts_ms: 0, bailed_Gambler: false, pot: sui::balance::zero<Asset>(), finalized: false, claim_amount_Gambler: 0, claimed_Gambler: false, Gambler_bet: 0, done_Gambler_bet: false, Race_winner: 0, done_Race_winner: false, action_Race_0_done: false, action_Gambler_1_done: false, action_Race_2_done: false };
        sui::transfer::share_object<Asset>(instance);
    }

    public entry fun join_Gambler<Asset>(instance: &mut Instance<Asset>, payment: sui::coin::Coin<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!(!instance.joined_Gambler, 100);
        instance.role_Gambler = sui::tx_context::sender(ctx);
    ...
    public entry fun move_Race_0<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!((sui::tx_context::sender(ctx) == instance.role_Race), 101);
    ...
    public entry fun move_Gambler_1<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, bet: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_Gambler), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
    ...
    public entry fun move_Race_2<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, winner: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_Race), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_Race = true;
        };
    ...
    public entry fun finalize<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!(instance.action_Race_2_done, 107);
    ...
    public entry fun claim_Gambler<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
