module montyhall::montyhall {
    ...
    public entry fun create_game<Asset>(timeout_ms: u64, ctx: &mut sui::tx_context::TxContext) {
        let instance = Instance<Asset> { id: sui::object::new(ctx), role_Host: 0x0, role_Guest: 0x0, joined_Host: false, joined_Guest: false, timeout_ms: timeout_ms, last_ts_ms: 0, bailed_Host: false, bailed_Guest: false, pot: sui::balance::zero<Asset>(), finalized: false, claim_amount_Host: 0, claimed_Host: false, claim_amount_Guest: 0, claimed_Guest: false, Host_car: 0, done_Host_car: false, Host_car_hidden: std::vector::empty<u8>(), done_Host_car_hidden: false, Guest_d: 0, done_Guest_d: false, Host_goat: 0, done_Host_goat: false, Guest_switch: false, done_Guest_switch: false, action_Host_0_done: false, action_Guest_1_done: false, action_Host_2_done: false, action_Guest_3_done: false, action_Host_4_done: false, action_Guest_5_done: false, action_Host_6_done: false };
        sui::transfer::share_object<Asset>(instance);
    }

    public entry fun join_Host<Asset>(instance: &mut Instance<Asset>, payment: sui::coin::Coin<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!(!instance.joined_Host, 100);
        instance.role_Host = sui::tx_context::sender(ctx);
    ...
    public entry fun join_Guest<Asset>(instance: &mut Instance<Asset>, payment: sui::coin::Coin<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!(!instance.joined_Guest, 100);
        instance.role_Guest = sui::tx_context::sender(ctx);
        instance.joined_Guest = true;
        sui::balance::join<Asset>(&mut instance.pot, sui::coin::into_balance<Asset>(payment));
        instance.last_ts_ms = sui::clock::timestamp_ms(clock);
    }

    public entry fun move_Host_0<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!((sui::tx_context::sender(ctx) == instance.role_Host), 101);
    ...
    public entry fun move_Guest_1<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!((sui::tx_context::sender(ctx) == instance.role_Guest), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
    ...
    public entry fun move_Host_2<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, hidden_car: vector<u8>) {
        assert!((sui::tx_context::sender(ctx) == instance.role_Host), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_Host = true;
        };
    ...
    public entry fun move_Guest_3<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, d: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_Guest), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_Guest = true;
        };
    ...
    public entry fun move_Host_4<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, goat: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_Host), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_Host = true;
        };
    ...
    public entry fun move_Guest_5<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, switch: bool) {
        assert!((sui::tx_context::sender(ctx) == instance.role_Guest), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_Guest = true;
        };
    ...
    public entry fun move_Host_6<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext, car: u64, salt: u64) {
        assert!((sui::tx_context::sender(ctx) == instance.role_Host), 101);
        if ((sui::clock::timestamp_ms(clock) > (instance.last_ts_ms + instance.timeout_ms))) {
            instance.bailed_Host = true;
        };
        ...
        let data_car = std::bcs::to_bytes<u64>(&car);
        std::vector::append<u8>(&mut data_car, std::bcs::to_bytes<u64>(&salt));
        assert!((sui::hash::keccak256(&data_car) == instance.Host_car_hidden), 106);
    ...
    public entry fun claim_Host<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
    ...
    public entry fun claim_Guest<Asset>(instance: &mut Instance<Asset>, clock: &sui::clock::Clock, ctx: &mut sui::tx_context::TxContext) {
        assert!(instance.finalized, 110);
