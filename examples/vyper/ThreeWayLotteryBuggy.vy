...
ACTION_Issuer_3: constant(uint256) = 3
ACTION_Issuer_4: constant(uint256) = 4
ACTION_Alice_5: constant(uint256) = 5
ACTION_Alice_6: constant(uint256) = 6
ACTION_Bob_7: constant(uint256) = 7
ACTION_Bob_8: constant(uint256) = 8
FINAL_ACTION: constant(uint256) = 8
...
@external
def move_Issuer_4(_c: int256, salt: uint256):
    assert self.role[msg.sender] == Role.Issuer, "bad role"
...
@external
def move_Alice_6(_c: int256, salt: uint256):
    assert self.role[msg.sender] == Role.Alice, "bad role"
...
@external
def move_Bob_8(_c: int256, salt: uint256):
    assert self.role[msg.sender] == Role.Bob, "bad role"
...
def distributePayoffs():
    ...
    self.balanceOf[self.address_Issuer] = 20 if ((not self.done_Alice_c) or (not self.done_Bob_c)) else (-10) if (not self.done_Issuer_c) else 20 if (self.Alice_c == self.Bob_c) else (-10) if (((((self.Alice_c + self.Bob_c) + self.Issuer_c) / 2) * 2) == ((self.Alice_c + self.Bob_c) + self.Issuer_c)) else (-10)
    self.balanceOf[self.address_Alice] = (-10) if ((not self.done_Alice_c) or (not self.done_Bob_c)) else 20 if (not self.done_Issuer_c) else (-10) if (self.Alice_c == self.Bob_c) else 20 if (((((self.Alice_c + self.Bob_c) + self.Issuer_c) / 2) * 2) == ((self.Alice_c + self.Bob_c) + self.Issuer_c)) else (-10)
    self.balanceOf[self.address_Bob] = (-10) if ((not self.done_Alice_c) or (not self.done_Bob_c)) else (-10) if (not self.done_Issuer_c) else (-10) if (self.Alice_c == self.Bob_c) else (-10) if (((((self.Alice_c + self.Bob_c) + self.Issuer_c) / 2) * 2) == ((self.Alice_c + self.Bob_c) + self.Issuer_c)) else 20
