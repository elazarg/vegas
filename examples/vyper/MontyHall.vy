...
def move_Host_4(_goat: int256):
    ...
    assert _goat != self.Guest_d, "where"
...
def move_Host_6(_car: int256, salt: uint256):
    ...
    assert self.Host_goat != _car, "where"
    self.Host_car = _car
...
def distributePayoffs():
    ...
    self.balanceOf[self.address_Host] = 0 if (self.done_Host_car and self.done_Guest_switch) else (-100) if (not self.done_Host_car) else 0
    self.balanceOf[self.address_Guest] = 20 if ((self.Guest_d != self.Host_car) == self.Guest_switch) else (-20) if (self.done_Host_car and self.done_Guest_switch) else 20 if (not self.done_Host_car) else (-100)
