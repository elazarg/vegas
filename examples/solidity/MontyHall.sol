...
contract MontyHall {
    ...
    function withdraw_Guest() public by(Role.Guest) action(Role.Guest, 7) depends(Role.Host, 6) {
        ...
        claimed_Guest = true;
        int256 payout = ((done_Host_car && done_Guest_switch) ? (((Guest_d != Host_car) == Guest_switch) ? 20 : (-20)) : ((!done_Host_car) ? 20 : (-100)));
        if (payout > 0) {
    ...
    function withdraw_Host() public by(Role.Host) action(Role.Host, 8) depends(Role.Host, 6) {
        ...
        claimed_Host = true;
        int256 payout = ((done_Host_car && done_Guest_switch) ? 0 : ((!done_Host_car) ? (-100) : 0));
        if (payout > 0) {
